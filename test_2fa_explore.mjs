...
try {
    ...
    // Look for "Enable" or "Set up" buttons related to MFA
    const enableBtns = btns.filter(b => 
        b.text.toLowerCase().includes('enable') || 
        b.text.toLowerCase().includes('set up') ||
        b.text.toLowerCase().includes('multi') ||
        b.text.toLowerCase().includes('authenticator')
    );
    console.log('\nMFA buttons:', JSON.stringify(enableBtns));

    // Try clicking "Enable" if found
    if (enableBtns.length > 0) {
        console.log('\nClicking MFA enable button...');
        const mfaBtn = page.locator(`button:has-text("${enableBtns[0].text.substring(0, 20)}")`).first();
        await mfaBtn.click({ timeout: 5000 });
        await sleep(5000);
        console.log('URL after enable:', page.url());
        await page.screenshot({ path: 'screenshots/2fa_11_after_enable.png', fullPage: true });
        
        // Check for QR code or setup key
        const text2 = await page.evaluate(() => document.body.innerText?.substring(0, 3000));
        console.log('Text after enable:', text2.substring(0, 800));
        
        // Look for images (QR code)
        ...
        // Look for setup key / secret
        const allText = await page.evaluate(() => document.body.innerText);
        const setupKeyMatch = allText.match(/[A-Z2-7]{16,64}/);
        if (setupKeyMatch) {
            console.log('Possible TOTP secret:', setupKeyMatch[0]);
        }

        // Look for the QR code data URL or otpauth:// URI
        const qrData = await page.evaluate(() => {
            const imgs = document.querySelectorAll('img');
            const results = [];
            imgs.forEach(img => {
                if (img.src && (img.src.includes('data:image') || img.src.includes('qr') || img.src.includes('totp'))) {
                    results.push(img.src.substring(0, 200));
                }
            });
            // Also check for any element with otpauth
            const html = document.documentElement.innerHTML;
            const otpauthMatch = html.match(/otpauth:\/\/[^"'\s<>]+/);
            if (otpauthMatch) results.push(otpauthMatch[0]);
            return results;
        });
        console.log('QR/TOTP data:', JSON.stringify(qrData));

        // Check all inputs on the page
        const inputs = await page.$$eval('input', els => els.map(el => ({
            type: el.type, name: el.name, placeholder: el.placeholder,
            ariaLabel: el.getAttribute('aria-label'), visible: el.offsetParent !== null, value: el.value?.substring(0, 50)
        })).filter(e => e.visible));
        console.log('Inputs:', JSON.stringify(inputs));
        
        // Check all buttons again
        const btns2 = await page.$$eval('button', els => els.map(el => ({
            text: el.textContent?.trim().substring(0, 80),
            visible: el.offsetParent !== null,
        })).filter(e => e.visible && e.text.length > 0));
        console.log('Buttons after enable:', JSON.stringify(btns2));
    }
